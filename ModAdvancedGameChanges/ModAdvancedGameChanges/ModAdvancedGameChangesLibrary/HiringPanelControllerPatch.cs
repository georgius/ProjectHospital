    ...
    public static class HiringPanelControllerPatch
    {
        ...
        public static bool SortTechnologistButtonsPrefix(bool clinic, HiringPanelController __instance)
        {
            ...
            if (ViewSettingsPatch.m_enabledTrainingDepartment)
            {
                ...
                if (departmentType == Database.Instance.GetEntry<GameDBDepartment>(Departments.Mod.TrainingDepartment))
                {
                    for (int i = 0; i < __instance.m_technologistCharacters.Count; i++)
                    {
                        __instance.m_technologistCharacters[i].SetActive(false);
                    ...
                    __instance.m_technologistCharacters[0].SetActive(true);
                    __instance.m_technologistCharacters[0].GetComponent<RectTransform>().anchoredPosition = new Vector3(0f, 0f, 0f);
